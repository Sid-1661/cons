...
describe("MasterChef", function () {
  before(async function () {
    ...
    this.CCToken = await ethers.getContractFactory("CCToken")
    this.ERC20Mock = await ethers.getContractFactory("ERC20Mock", this.minter)
  })

  beforeEach(async function () {
    this.cc = await this.CCToken.deploy()
    await this.cc.deployed()
  })

  it("should set correct state variables", async function () {
    this.chef = await this.MasterChef.deploy(this.cc.address, this.dev.address, "1000", "0", "1000")
    await this.chef.deployed()

    await this.cc.transferOwnership(this.chef.address)

    const cc = await this.chef.cc()
    const devaddr = await this.chef.devaddr()
    const owner = await this.cc.owner()

    expect(cc).to.equal(this.cc.address)
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    it("should allow emergency withdraw", async function () {
      ...
      this.chef = await this.MasterChef.deploy(this.cc.address, this.dev.address, "100", "100", "1000")
      await this.chef.deployed()
    ...
    it("should give out cc only after farming time", async function () {
      // 100 per block farming rate starting at block 100 with bonus until block 1000
      this.chef = await this.MasterChef.deploy(this.cc.address, this.dev.address, "100", "100", "1000")
      await this.chef.deployed()
      ...
      expect(await this.cc.balanceOf(this.bob.address)).to.equal("0")
      ...
      expect(await this.cc.balanceOf(this.bob.address)).to.equal("1000")

      await advanceBlockTo("104")
      await this.chef.connect(this.bob).deposit(0, "0") // block 105

      expect(await this.cc.balanceOf(this.bob.address)).to.equal("5000")
      expect(await this.cc.balanceOf(this.dev.address)).to.equal("500")
      expect(await this.cc.totalSupply()).to.equal("5500")
    })

    it("should not distribute cc if no one deposit", async function () {
      // 100 per block farming rate starting at block 200 with bonus until block 1000
      this.chef = await this.MasterChef.deploy(this.cc.address, this.dev.address, "100", "200", "1000")
      await this.chef.deployed()
      await this.cc.transferOwnership(this.chef.address)
      await this.chef.add("100", this.lp.address, true)
      await this.lp.connect(this.bob).approve(this.chef.address, "1000")
      await advanceBlockTo("199")
      expect(await this.cc.totalSupply()).to.equal("0")
      ...
      expect(await this.cc.totalSupply()).to.equal("0")
      expect(await this.cc.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.cc.balanceOf(this.dev.address)).to.equal("0")
      ...
      expect(await this.cc.totalSupply()).to.equal("11000")
      expect(await this.cc.balanceOf(this.bob.address)).to.equal("10000")
      expect(await this.cc.balanceOf(this.dev.address)).to.equal("1000")
      expect(await this.lp.balanceOf(this.bob.address)).to.equal("1000")
    })

    it("should distribute cc properly for each staker", async function () {
      // 100 per block farming rate starting at block 300 with bonus until block 1000
      this.chef = await this.MasterChef.deploy(this.cc.address, this.dev.address, "100", "300", "1000")
      await this.chef.deployed()
      await this.cc.transferOwnership(this.chef.address)
      await this.chef.add("100", this.lp.address, true)
      ...
      await this.chef.connect(this.alice).deposit(0, "10", { from: this.alice.address })
      expect(await this.cc.totalSupply()).to.equal("11000")
      expect(await this.cc.balanceOf(this.alice.address)).to.equal("5666")
      expect(await this.cc.balanceOf(this.bob.address)).to.equal("0")
      expect(await this.cc.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.cc.balanceOf(this.chef.address)).to.equal("4334")
      expect(await this.cc.balanceOf(this.dev.address)).to.equal("1000")
      // Bob withdraws 5 LPs at block 330. At this point:
      //   Bob should have: 4*2/3*1000 + 2*2/6*1000 + 10*2/7*1000 = 6190
      await advanceBlockTo("329")
      await this.chef.connect(this.bob).withdraw(0, "5", { from: this.bob.address })
      expect(await this.cc.totalSupply()).to.equal("22000")
      expect(await this.cc.balanceOf(this.alice.address)).to.equal("5666")
      expect(await this.cc.balanceOf(this.bob.address)).to.equal("6190")
      expect(await this.cc.balanceOf(this.carol.address)).to.equal("0")
      expect(await this.cc.balanceOf(this.chef.address)).to.equal("8144")
      expect(await this.cc.balanceOf(this.dev.address)).to.equal("2000")
      ...
      expect(await this.cc.totalSupply()).to.equal("55000")
      expect(await this.cc.balanceOf(this.dev.address)).to.equal("5000")
      // Alice should have: 5666 + 10*2/7*1000 + 10*2/6.5*1000 = 11600
      expect(await this.cc.balanceOf(this.alice.address)).to.equal("11600")
      // Bob should have: 6190 + 10*1.5/6.5 * 1000 + 10*1.5/4.5*1000 = 11831
      expect(await this.cc.balanceOf(this.bob.address)).to.equal("11831")
      // Carol should have: 2*3/6*1000 + 10*3/7*1000 + 10*3/6.5*1000 + 10*3/4.5*1000 + 10*1000 = 26568
      expect(await this.cc.balanceOf(this.carol.address)).to.equal("26568")
    ...
    it("should give proper cc allocation to each pool", async function () {
      // 100 per block farming rate starting at block 400 with bonus until block 1000
      this.chef = await this.MasterChef.deploy(this.cc.address, this.dev.address, "100", "400", "1000")
      await this.cc.transferOwnership(this.chef.address)
      ...
      expect(await this.chef.pendingCC(0, this.alice.address)).to.equal("10000")
      // Bob deposits 10 LP2s at block 425
      await advanceBlockTo("424")
      await this.chef.connect(this.bob).deposit(1, "5", { from: this.bob.address })
      // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
      expect(await this.chef.pendingCC(0, this.alice.address)).to.equal("11666")
      await advanceBlockTo("430")
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      expect(await this.chef.pendingCC(0, this.alice.address)).to.equal("13333")
      expect(await this.chef.pendingCC(1, this.bob.address)).to.equal("3333")
    })

    it("should stop giving bonus cc after the bonus period ends", async function () {
      // 100 per block farming rate starting at block 500 with bonus until block 600
      this.chef = await this.MasterChef.deploy(this.cc.address, this.dev.address, "100", "500", "600")
      await this.cc.transferOwnership(this.chef.address)
      await this.lp.connect(this.alice).approve(this.chef.address, "1000", { from: this.alice.address })
      ...
      expect(await this.chef.pendingCC(0, this.alice.address)).to.equal("10500")
      // At block 606, Alice withdraws all pending rewards and should get 10600.
      await this.chef.connect(this.alice).deposit(0, "0", { from: this.alice.address })
      expect(await this.chef.pendingCC(0, this.alice.address)).to.equal("0")
      expect(await this.cc.balanceOf(this.alice.address)).to.equal("10600")
    })
  })
})
